...
默认提交截止日为 `2026-08-02`。如果候选起始日不是交易日，代码会向后跳到合理交易日。正式提交默认启用 `BDC_DATA_CUTOFF_DATE=2026-07-27`，训练和预测都会忽略晚于该日期的数据，以保证本地 `result.csv` 和赛方复现口径一致。`--as-of-date` 仅用于本地调试的数据截断，不能晚于提交截止日。
