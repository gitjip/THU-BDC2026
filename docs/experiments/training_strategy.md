...
`v1.6.0 noid-rank-cleanrisk` 已跑完 24 窗口，均值和最差窗口都明显变差，说明清洗启发的流动性/回撤风险特征这条线暂时不值得继续。下一步回到 `rank-replace` 主线，先测试 `v1.6.1 noid-rank-multiperiod`：只追加 3/5/10/20/40 日多周期基础特征，用最近 3 窗口判断是否值得扩到 12/24 窗口。
