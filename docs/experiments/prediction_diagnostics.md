...
当前判断：`rank-replace` 的主要问题是源模型排序信号在某些窗口失效，而不是风险后处理没调好。下一步不要把单模型低波动、低风险、过热惩罚或前缀限制接成默认；更值得做的是改善模型输入信号或训练目标本身。`v1.6.0 noid-rank-cleanrisk` 已在 24 窗口明显变差，当前对应的小步特征实验改为 `v1.6.1 noid-rank-multiperiod`。
