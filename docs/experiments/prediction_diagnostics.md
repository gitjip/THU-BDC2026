...
`v1.12.0 LightGBM` 和 `v1.12.1 ensemble-lowoverheat` 都不是默认主线，但它们暴露出一个有用现象：防守候选在不少 Transformer 低分窗口更稳，在 Transformer 高分窗口又经常拖累收益。

为避免直接把防守策略固定接入，`v1.12.2` 新增离线门控评估：
...
第一版只使用预测日前历史信号，例如 Transformer top5 的短期过热均值、波动均值、与 LightGBM 的 top5/top20 重合数量，以及近 5 日市场状态。真实未来收益只用于事后评分。

24 窗口结论：

- 最好规则 `v1.12.1 / gate_tf_overheat_ge_0p7` 把均值从 `0.017557` 提到约 `0.029874`；
- 正分窗口从 `14/24` 提到 `17/24`；
- 高分窗口切防守比例约 `0.300`，没有明显大面积误切；
- 但最差窗口仍为 `-0.066987`，没有改善。

因此，这个结果说明“过热时防守”有明显诊断价值，但不能直接作为提交策略。后续若继续门控方向，应把重点放在最差窗口识别上，例如市场弱势、短期过热与模型分歧的更小组合规则。

`v1.12.3` 起，门控复盘改用 [evaluation_metrics.md](evaluation_metrics.md) 的统一口径：均值和配对均值差是主目标，`worst3_mean_score` 与 `cvar_20_score` 是主要风险指标，单个 `min_score` 只作为警报，不再作为所有实验的一票否决。

按新口径重新看，`v1.12.1 / gate_tf_overheat_ge_0p7` 均值、CVaR20、worst3、正分窗口和高分误切约束都优于或不弱于主线，属于“默认候选”级别。但它仍是离线门控结果，必须另起版本接入实际预测流程后再判断是否能改默认提交。

`v1.12.4 ensemble-gate-overheat` 已完成 24 窗口 walk-forward 复现：

- 均值约 `0.018227`，略高于旧 `v1.4.5 rank-replace` 的 `0.017557`；
- worst3 约 `-0.048249`，CVaR20 约 `-0.041475`，都好于旧 `v1.4.5`；
- 正分窗口 `15/24`，也略高于旧 `v1.4.5` 的 `14/24`；
- 平均 Spearman 约 `-0.079064`，与 `v1.12.1` 接近，弱于 `v1.12.0`；
- top20/top50 后验收益约 `0.003482/0.000621`，与 `v1.12.1` 接近，好于旧 `v1.4.5`。

结论：门控复现比固定低过热防守更合理，但收益提升很小，且当前代码重跑源模型与旧预测文件存在漂移；先保留候选，不切默认。
...
当前判断：`rank-replace` 的主要问题是源模型排序信号在某些窗口失效，而不是风险后处理没调好。下一步不要把单模型低波动、低风险、过热惩罚或前缀限制接成默认；更值得做的是改善模型输入信号或训练目标本身。`v1.6.0 noid-rank-cleanrisk` 和 `v1.6.1 noid-rank-multiperiod` 都已在 24 窗口明显变差，后续不要继续简单追加成组基础特征。
