...
## v1.13.2-v1.13.4 同口径结果

这轮已经按 `v1.13.1` 规范重跑，源实验 `data_signature` 均匹配，目标窗口均为同一批 24 个连续 5 交易日窗口：

- `v1.13.0 noid-rank-replace`：当前 Transformer 主基线，均值约 `0.007778`，worst3 约 `-0.065648`，CVaR20 约 `-0.052621`，正分窗口 `11/24`；
- `v1.13.2 noid-rank-lgbm`：LightGBM 防守源，均值约 `0.009367`，worst3 约 `-0.060486`，CVaR20 约 `-0.049518`，正分窗口 `16/24`；
- `v1.13.3 ensemble-gate-overheat`，阈值 `0.70`：均值约 `0.019635`，worst3 约 `-0.058833`，CVaR20 约 `-0.046815`，正分窗口 `13/24`；
- `v1.13.4 ensemble-gate-overheat`，阈值 `0.65`：均值约 `0.021579`，worst3 约 `-0.058833`，CVaR20 约 `-0.046815`，正分窗口 `15/24`。

相对 `v1.13.0`：

- `v1.13.3` 平均提升约 `+0.011857`；
- `v1.13.4` 平均提升约 `+0.013801`，胜/负/平为 `9/1/14`，`top2_positive_diff_share` 约 `0.427`；
- `v1.13.4` 比 `v1.13.3` 只多改善两个窗口，平均提升约 `+0.001943`，这部分改善集中在阈值从 `0.70` 降到 `0.65` 后新增触发的窗口。
