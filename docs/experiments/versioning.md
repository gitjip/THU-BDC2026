...
本项目用 `vMAJOR.MINOR.PATCH` 记录一次完整实验，例如 `v1.0.0`、`v1.3.10`。

每一段都可以是多位数字，不要把它理解成“每段只能到 9”的计数器；版本号应按实验语义递增。
...
`--resume` 只适合中断续跑同一个实验。更新 `stock_data`、改变窗口数量/步长或切换 profile 时，应创建新版本；流程会拒绝把这些变化混进旧版本目录。
