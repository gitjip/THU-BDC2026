...
推荐使用 Python 3.10 到 3.12。项目依赖由 `pyproject.toml` 和 `uv.lock` 固定；当前锁文件中的主要版本包括 Python `>=3.10,<3.13`、PyTorch `2.10.0`（Linux CUDA 环境为 `2.10.0+cu128`）、pandas `2.3.2`、numpy `2.2.6/2.3.3`、scikit-learn `1.7.2`、LightGBM `4.7.0` 和 TA-Lib `0.6.8`。不同 Python 小版本可能选择对应的 numpy 构建版本，但应始终使用 `uv.lock` 安装。
...
训练、Transformer 预测、LightGBM 预测和集成重排都会忽略晚于 `2026-07-27` 的数据，确保本地结果与赛方复现口径一致。若明确要使用更新数据，必须显式覆盖该变量并重新训练、预测；设置为空值可以解除截止锁。
...
调试模式会进一步限制训练目标日数量和每日股票数量，以缩短本地运行时间。正式默认强度为 `BDC_SUBMISSION_STRENGTH=strong`：Transformer 使用全部可打标签目标日和全部股票，LightGBM 使用最近 240 个训练目标日和全部股票。若需要复现 `v1.13.4` 同口径实验，可设置 `BDC_SUBMISSION_STRENGTH=validated`；`max` 只作为隔离的高强度实验，不是当前默认提交路径。
...
2. 分别对全部候选股票打分；
3. 按 `BDC_DATA_CUTOFF_DATE` 读取不晚于 `2026-07-27` 的历史行情数据；
...
正式提交复现时不需要额外设置截止日期，因为默认已经锁定到 `2026-07-27`。如需显式确认当前配置，可运行：
...
- 当前正式候选使用 `strong` 双模型门控集成；`max` 训练结果仅作本地对照，不应覆盖正式 `output/result.csv`。
