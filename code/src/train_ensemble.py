...
from ensemble_config import SUBMISSION_STRENGTH, get_submission_ensemble_sources
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="使用小模型快速检查集成训练流程"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="只打印将要训练的源模型配置"
    )
...
def source_train_script(env: dict[str, str]) -> str:
    model_kind = env.get("BDC_MODEL_KIND", "transformer").strip().lower()
    if model_kind == "lgbm":
        return "code/src/train_lgbm.py"
    if model_kind in {"", "transformer"}:
        return "code/src/train.py"
...
def run_source_train(label: str, env: dict[str, str]) -> float:
    ...
    command = [sys.executable, source_train_script(env)]
    logger.info("")
    logger.info("=" * 72)
    logger.info(
        "训练集成源模型: %s | kind=%s -> %s",
...
def main() -> None:
    ...
    if not debug:
        logger.info("正式训练强度: %s", SUBMISSION_STRENGTH)
    logger.info("源模型数量: %s", len(sources))
    for source in sources:
        logger.info(
            "源模型: %s | kind=%s | output_dir=%s",
            source.label,
            source.env.get("BDC_MODEL_KIND", "transformer"),
            source.output_dir,
        )
    ...
    logger.info("")
    logger.info(
        "提交集成训练完成: 源模型耗时合计=%s | 总耗时=%s",
        format_duration(total_source_seconds),
