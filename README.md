...
训练产物由本项目 `train.sh` 从输入数据重新训练得到，主要保存到 `model/` 目录。默认正式提交使用两模型集成，完整训练会生成两个源模型目录：

```text
model/ensemble/noid/
model/ensemble/noid_rank_replace/
```
...
本项目把股票选择建模为横截面排序任务。对每个目标交易日，模型同时看到多只股票各自过去一段时间的量价和技术指标序列，输出每只股票的排序分数。正式预测默认训练两个配置不同的排序模型，分别输出 top5，再从两个 top5 的并集中按提交日前历史低波动优先重排选回 5 只股票，当前默认等权重 `0.2`。
...
- 正式提交入口支持两模型集成：`noid` 和 `noid_rank_replace` 各自产生模型分数，再做 top5 并集低波动重排。
...
部分实验还支持横截面 rank 特征、市场相对特征和移除股票编号特征，这些由环境变量控制。当前 `train.sh` 默认集成源模型使用 39 特征，并移除 `instrument` 股票编号输入。
...
- `noid`：39 个基础特征，移除 `instrument` 股票编号特征；
- `noid_rank_replace`：在 `noid` 基础上，用当日横截面 rank 替换部分绝对量价尺度特征。

预测时两个源模型分别对同一份 `stock_data` 生成完整候选排名，各自先取 top5。然后代码取两个 top5 的并集，使用提交截止日前已知历史数据计算最近 20 个交易日收益波动率，在并集内按低波动优先选回 5 只股票并等权输出。该流程不使用未来标签，也不硬编码股票代码。
...
2. 分别使用 `noid` 和 `noid_rank_replace` 两个源模型对全部候选股票打分；
3. 读取不晚于提交截止日的历史行情数据；
4. 根据提交截止日寻找未来连续 5 个合理交易日；
5. 取两个源模型 top5 并集；
6. 用提交截止日前已知历史数据计算候选股票最近 20 个交易日波动率；
7. 在并集中按低波动优先选回 5 只股票；
...
最多 5 只股票，权重和不超过 1。当前默认策略是两模型 top5 并集低波动重排后等权重 `0.2`。`output/result_scores.csv` 是完整候选排名诊断文件，不是提交文件。
