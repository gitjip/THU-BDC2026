...
6. 使用训练好的 `best_model.pth` + `scaler.pkl` 对提交截止日之后的未来5个交易日生成Top5选股结果。
...
默认提交截止日为 `2026-08-02`，预测窗口从之后第一个合理交易日开始，默认是 `2026-08-03` 到 `2026-08-07`。如果候选起始日不是交易日，代码会向后跳到合理交易日。`--as-of-date` 仅用于本地调试的数据截断，不能晚于提交截止日。

注意：正式预测窗口没有真实标签，不能提前校验模型好坏；平时只能用历史窗口验证或 walk-forward 估计表现。
