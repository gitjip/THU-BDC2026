...
本项目把股票选择建模为横截面排序任务。对每个目标交易日，模型同时看到多只股票各自过去一段时间的量价和技术指标序列，输出每只股票的排序分数。预测时默认按分数降序选择前 5 只股票，当前默认等权重 `0.2`。
...
- 支持可选预测后处理，例如从高分候选池中按历史低波动优先选回 top5；默认不启用。
...
该策略会先取模型分数前 10 只作为候选池，再按提交截止日前最近 20 个交易日的历史波动率从低到高选回 5 只。它只是实验开关，默认提交流程仍使用 `model_top5`。
